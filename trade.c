...
void set_stock_quantities(unsigned char *quant)
{
	int i;
...
void scoop_item (unsigned char un)
{
	short int type;
	unsigned char trade;
	...
	if (!(cmdr->equipment & FUEL_SCOOP) || (universe[un].location.y >= 0) ||
		(total_cargo() == cmdr->cargo_capacity))
	{
		explode_object (un);
		damage_ship (128 + (universe[un].energy>>1), universe[un].location.z > 0);
		return;
	}
	...
	if (ELITE_SHIPS.pShipDataTable[type].pShipData->scoop_type != 0)
	{
		trade = ELITE_SHIPS.pShipDataTable[type].pShipData->scoop_type + 1;
		cmdr->current_cargo[trade]++;
		info_message (stock_market[trade].name);
		remove_ship (un);
		return;					
	...
	explode_object (un);
	damage_ship (universe[un].energy>>1, universe[un].location.z > 0);
}
